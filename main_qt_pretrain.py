...
from utils_.face_recognition_pre import FaceRecognition
        ...
        for obj in objs:
            ...
            if iou < 0.2:
                ...
                if now_pos \
                        ...
                        and my_utils.distance(self.last_center_point, detect_center) > 5:
...
class OpenCVWindow(QMainWindow):
    ...
    def update_frame(self):
        ...
        # 识别时间：2s
        if self.frame_counter == 30 * 1 and self.mode == 2:
            if len(self.name_list) < 10:
                self.show_message("识别失败", f"您未录入此系统")
            else:
                counted_elements = Counter(self.name_list)
                most_common_element, _ = counted_elements.most_common(1)[0]

                self.show_message("识别成功", f"识别结果：{most_common_element}")
            self.frame_counter = 0
            self.mode = 0
        ...
        if ret:
            ...
            # 识别模式
                ...
                for idx, obj in enumerate(objs):
                    transformed = transform_face(frame, obj.landmark, smoothing_window=1)
                    # TODO 继续放到dbface
                    if len(detect(self.dbface, transformed)) == 0:
                        continue
                    cv2.imshow(f"transformed{idx}", cv2.cvtColor(transformed, cv2.COLOR_RGB2BGR))
                    name, max_times = self.recognition(transformed)

                    cv2.putText(frame, 'identifying...', (20, 20), 0, 1, (255, 0, 0), 1, 16)
                    common.drawbbox_rec(frame, obj, name, max_times, time_threshold)
                    if max_times > time_threshold:
