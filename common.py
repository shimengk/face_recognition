...
def drawbbox(image, bbox, color=None, thickness=2, textcolor=(0, 0, 0), landmarkcolor=(0, 255, 255)):
...
num = 0
# (239, 121, 162)
def drawbbox_rec(image, bbox, name, sim, threshold, color=None, thickness=2, textcolor=(0, 0, 0),
    ...
    text = f"{name} " if sim > threshold else 'null'
    color = (0, 255, 0) if sim > threshold else (0, 0, 255)
    if name == 'recing':
        text = 'recing'
        color = (0, 0, 255)
    # print(text+str(round(sim, 2)))
    print(text)
    x, y, r, b = intv(bbox.box)
    w = r - x + 1
    h = b - y + 1
    ...
    #     if w > h:
    #         offset = (w - h) // 2
    #         crop_img = image[y:y + h, x + offset:x + w - offset]
    #     else:
    #         offset = (h - w) // 2
    #         crop_img = image[y + offset:y + h - offset, x:x + w]
    #     try:
    #         crop_img = cv2.resize(crop_img, (224, 224))
    #         cv2.imwrite(f'res/{str(num)}.jpg', crop_img)
    #
    #     except:
    #         print('save error')
    num += 1
    cv2.rectangle(image, (x, y, r - x + 1, b - y + 1), color, thickness, 16)
    # cv2.circle(image, (x+w//2, y+h//2), w//2, color, thickness, 16)
